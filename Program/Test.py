...
def tes():
    ...
    # out = mlr.MyChain(n_in=db.p['n_h_neuron'], n_bypass=db.p['n_in'], n_l1=db.p['out_l1'],
    #                   n_l2=db.p['out_l2'], n_l3=db.p['out_l3'], n_top=db.p['out_top'],
    #                   nor_w=db.p['out_lecnor_ws'], uni_w=db.p['out_uni_ws'],
    ...
    tsk = fd.Field(field_x=db.p['field_x'], field_y=db.p['field_y'],
                   Num_plot=len(ap.keys()), fig_number=0, rew=db.p['reward'],
                   pena=db.p['wall_penalty'], spena=db.p['pena_without_switching'],
                   testing=True, sw_scale=db.p['sw_scale'])
    ...
    for num_ep, ep in enumerate(ap.keys()):
        ...
        for step in range(db.p['max_step']):
            ...
            if db.p['moving_ave']:
                if step == 0:
                    mma_x, mma_y = out_o['top'].data[0][1], out_o['top'].data[0][2]
                else:
