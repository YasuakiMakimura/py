...
class Field:
    def __init__(self, field_x=20.0, field_y=20.0, agent_x=10.0, agent_y=1.0,
...
# %%%%%"今の座標をリストに入れるための関数"%%%%% #ただし最初にこの関数を呼び出した時はdefaultで初期位置が格納されている
    ...
    def ini_add_p_log_test(self, posi_x: float, posi_y: float, var_ep: int, color=('#FF6347', '#008B8B', '#8B008B', '#FF1493', '#008000'), ms=10):
        self.traj_x.append(posi_x)
        self.traj_y.append(posi_y)
        self.ini_trajectries[var_ep], = self.frame.plot(self.traj_x[:], self.traj_y[:], ms=ms, marker='o', c=color[var_ep])
...
# %%%%%"座標情報をプロット情報としてリストに格納する関数"%%%%%
    ...
    def replot_test(self, var_ep: int, color=('#FF6347', '#008B8B', '#8B008B', '#FF1493', '#008000'), lw=1, ms=5):
        self.trajectries[var_ep], = self.frame.plot(self.traj_x[:], self.traj_y[:], ms=ms, marker='o', c=color[var_ep], lw=lw)
        self.traj_x = []
        self.traj_y = []
        pause(display_pace)
...
# %%%%%"ゴールとエージェントの初期座標をランダムで決めて座標格納リストに入れる関数"%%%%%
    def all_p_random_set(self):
        ...
        while True:
            ...
            if dist_g1 >= self.radius_goal and dist_g2 >= self.radius_goal and dist_s >= self.radius_switch:
                break
            # if dist_g1 >= self.radius_goal+4.0 and dist_g2 >= self.radius_goal and dist_s >= self.radius_switch:
            #     break
        # self.add_p_log(self.x_agent, self.y_agent)
...
# %%%%%"現在のエージェントの状態を返す関数(返り値はそのままネットワークの入力情報として使用する)"%%%%%
    def get_state(self):
        ...
        # print(ret_gsin1, ret_gcos1, ret_gsin2, ret_gcos2, ret_swsin, ret_swcos)
