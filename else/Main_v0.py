...
class Database:
    def __init__(self, nm_dir_datalist: str = 'data', b_save: bool = True):
        ...
        self.p['path_dir_count'] = f'{self.p["path_dir_data"]}count/'
        os.makedirs(self.p['path_dir_count'], exist_ok=True)
...
if __name__ == "__main__":
    ...
    def plot_after_eploop():
        ...
        # print(f'sc:{sc}')
        pie.out([rc, pc, sc])
        pie.save(f'{db.p["path_dir_count"]}count')
        pickle_v0.w(list_exp_x, f'{db.p["path_dir_exp"]}expx')
        pickle_v0.w(list_exp_y, f'{db.p["path_dir_exp"]}expy')
    ...
    # import Analysis.moving_average_v0
    import pickle_v0
    ...
    ltau = log.Log(n_line=db.p['n_pl_ltau'], num_fig=5, subplot=True, y_low=-1.1, y_high=1.1,
                   figsize=(8, 6), x_high=db.p['max_step'], inf=db.p['v_tau'][0],
                   c='blue', xlabel='episode', ylabel='output(τ: 10)')
