...
class Reservoir:
    def __init__(self, num_in: int, num: int, num_out: int, v_tau: Tuple[Union[int, float]], n_tau: Tuple[int], p_connect: int = 10, v_lambda: float = 1.8, in_wscale=1.0, fb_wscale=1.0):
        ...
        self.inf_con = xp.array(xp.random.choice(2, (num, num), p=[(100 - p_connect) / 100, p_connect / 100]))
        ...
        self.rw = v_lambda * xp.random.normal(0, 1 / math.sqrt(float(num * p_connect / 100)), (num, num))
    ...
    def reset_net(self):
        ...
        self.ru = xp.random.uniform(-0.01, 0.01, self.num)
        # self.ru = xp.random.normal(0, 0.5, self.num)
        # self.ru = xp.random.normal(0, 0.1, self.num)
        self.ro = xp.tanh(self.ru)
