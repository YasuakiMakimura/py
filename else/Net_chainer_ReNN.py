...
#####"ネットワークの構築"#####
class MyChain(Chain):
    def __init__(self, n_in, n_bypass, n_l1, n_l2, n_l3, n_top, nor_w=(False, (None, None, None, (None, None))), uni_w=(False, (None, None, None, (None, None))), uni_bypass_w=(False, None), nobias=(False, False, False, False, (False, False))):
        initializer_out = []
        ...
        if uni_w[0]:
            ...
            for iii in range(len(nor_w[1])):
                initializer_out.append(initializers.HeNormal(scale=nor_w[1][iii]))
        else:
        ...
        super(MyChain, self).__init__(
            ...
            lout_critic = L.Linear(n_l3, 1, nobias=nobias[4][0], initialW=initializer_out[3][0]),
            lout_actor = L.Linear(n_l3, 2, nobias=nobias[4][1], initialW=initializer_out[3][1]),
...
# %%%%%"階層型NNの各層での出力を求める関数(戻り値は各層の出力)"%%%%%
    def ff_comp(self, net_in, bypass, net_type=1):# net_in:リザバから階層型NNへの入力、　bypass:入力層からのバイパス
        if net_type == 1:
            ...
            critic = F.tanh(self.lout_critic(h3))
            actor = F.tanh(self.lout_actor(h3))
            return h1, h2, h3, critic, actor
